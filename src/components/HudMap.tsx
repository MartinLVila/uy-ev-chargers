...
import { buildCorridorPaths, buildCountryPaths, buildLocalityPoints } from "@/lib/ui/hud-map";
...
export function HudMap({ stations }: { stations: StationStatus[] }) {
  ...
  const corridors = buildCorridorPaths(projection);

  return <HudMapView paths={paths} points={points} corridors={corridors} />;
}
